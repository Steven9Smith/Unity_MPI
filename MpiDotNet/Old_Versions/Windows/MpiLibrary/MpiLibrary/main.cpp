...
int mpi_tests(int argc, char *argv[]){
    ...
    int world_size = get_world_size__ws();
	int world_rank = get_world_rank__wr();
    // print results
...
#pragma endregion
        ...
        printf("] with tag '%d'.\n", tag);
        mpi_send_int_array_s(int_buffer_array,array_size, 1, tag);
        tag++;
        //Send char test  
        ...
        printf("] with tag '%d'.\n", tag);
        mpi_send_char_array_s(char_buffer_array,array_size, 1, tag);
        tag++;
        //Send float test 
        ...
        printf("] with tag '%d'.\n", tag);
        mpi_send_float_array_s(float_buffer_array, array_size, 1, tag);
        tag++;
        /*
        //Send double test
...
#pragma region 
        ...
        mpi_recv_int_array_s(tmp_int_buffer_array,array_size, 0, tag,status_count_lo,status_count_hi_and_cancelled,status_MPI_SOURCE,
            status_MPI_TAG,status_MPI_ERROR);
        ...
        mpi_recv_char_array_s(tmp_char_buffer_array,array_size, 0, tag,status_count_lo,status_count_hi_and_cancelled,status_MPI_SOURCE,
            status_MPI_TAG,status_MPI_ERROR);
        ...
        mpi_recv_float_array_s(tmp_float_buffer_array,array_size, 0, tag,status_count_lo,status_count_hi_and_cancelled,status_MPI_SOURCE,
            status_MPI_TAG,status_MPI_ERROR);
